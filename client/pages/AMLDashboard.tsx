...
export default function AMLDashboard() {
  ...
  const { fillAddressFromConnectedWallet, isConnected } = useWalletIntegration();
  ...
  const handleInvestigation = async () => {
    ...
    try {
      const result = await mcpApiClient.comprehensiveInvestigation(address, investigationType, userQuery);
      setResults(result);
    } catch (err) {
  ...
  const handleWalletScreening = async () => {
    ...
    try {
      const result = await mcpApiClient.screenWalletComprehensive(address, chain);
  ...
  const handleTransactionAnalysis = async () => {
    ...
    try {
      const result = await mcpApiClient.analyzeTransactionsDetailed(address, chain, 100);
      setResults({ ...results, transactions: result });
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Transaction analysis failed');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const getRiskIcon = (riskLevel: string) => {
    switch (riskLevel?.toLowerCase()) {
      case 'critical': return <BlockIcon className="h-4 w-4" />;
      case 'high': return <WarningIcon className="h-4 w-4" />;
      case 'medium': return <InfoIcon className="h-4 w-4" />;
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center space-y-4 sm:space-y-0">
          <div>
            ...
            <p className="text-muted-foreground">Comprehensive blockchain compliance analysis powered by AI</p>
          </div>
          <div className="flex items-center gap-2">
            <Badge variant="outline" className="border-brand-light text-brand-light">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Comprehensive AML analysis using MetaSleuth, Chainabuse, Helius, Coinstats, and PEP databases
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div className="lg:col-span-2 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <Select value={investigationType} onValueChange={setInvestigationType}>
                      ...
                      <SelectContent>
                        ...
                        <SelectItem value="tracing">Transaction Tracing</SelectItem>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                    <Button onClick={handleInvestigation} disabled={isLoading} className="bg-brand-light">
                      {isLoading ? <RefreshIcon className="mr-2 h-4 w-4 animate-spin" /> : <SearchIcon className="mr-2 h-4 w-4" />}
                      Full Investigation
                    </Button>
                    <Button onClick={handleWalletScreening} disabled={isLoading} variant="outline">
                      <SecurityIcon className="mr-2 h-4 w-4" />
                      Screen Wallet
                    </Button>
                    <Button onClick={handleTransactionAnalysis} disabled={isLoading} variant="outline">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                      <div className="text-center p-4 border rounded-lg">
                        <div className="flex items-center justify-center mb-2">
                          ...
                          <span className="ml-2 text-lg font-bold">Risk Level</span>
                        </div>
                      ...
                      <div className="text-center p-4 border rounded-lg">
                        <div className="text-lg font-bold text-brand-light">{results.riskScore || 'N/A'}</div>
                        <div className="text-sm text-muted-foreground">Risk Score</div>
                      </div>
                      <div className="text-center p-4 border rounded-lg">
                        <div className="text-lg font-bold text-warning-amber">{results.findings?.length || 0}</div>
              ...
              <Card>
                ...
                <CardContent>
                  {results.screening ? (
                        ...
                        <div className="p-4 border rounded-lg">
                          <div className="flex items-center justify-between">
                            ...
                            {results.screening.riskAssessment.sanctionsMatch ? (
                              <Badge className="bg-red-500 text-white">MATCH</Badge>
                            ) : (
                        ...
                        <div className="p-4 border rounded-lg">
                          <div className="flex items-center justify-between">
                            ...
                            {results.screening.riskAssessment.blacklistMatch ? (
                              <Badge className="bg-red-500 text-white">LISTED</Badge>
                            ) : (
                              <Badge className="bg-green-500 text-white">CLEAR</Badge>
                            )}
                          </div>
                      ...
                      {results.screening.recommendations && (
                        <div>
                          ...
                          <ul className="space-y-1">
                            {results.screening.recommendations.map((rec: string, index: number) => (
                    ...
                    <p className="text-muted-foreground">Run wallet screening to see sanctions results</p>
                  )}
                </CardContent>
              </Card>
            </TabsContent>
              ...
              <Card>
                ...
                <CardContent>
                  {results.transactions ? (
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div className="text-center p-3 bg-muted rounded-lg">
                          <div className="text-lg font-bold">{results.transactions.summary.totalTransactions}</div>
                          <div className="text-sm text-muted-foreground">Total Transactions</div>
                        </div>
                        <div className="text-center p-3 bg-muted rounded-lg">
                          <div className="text-lg font-bold">{results.transactions.summary.uniqueCounterparties}</div>
                          <div className="text-sm text-muted-foreground">Unique Counterparties</div>
                        </div>
                        <div className="text-center p-3 bg-muted rounded-lg">
                          <div className="text-lg font-bold">{results.transactions.summary.totalVolume}</div>
                          <div className="text-sm text-muted-foreground">Total Volume</div>
                        </div>
                        <div className="text-center p-3 bg-muted rounded-lg">
                          <div className="text-lg font-bold">{results.transactions.summary.timespan?.spanDays || 'N/A'}</div>
                          <div className="text-sm text-muted-foreground">Active Days</div>
                        </div>
                      </div>

                      {results.transactions.patterns && results.transactions.patterns.length > 0 && (
                        <div>
                          <h4 className="font-medium mb-2">Detected Patterns</h4>
                          <div className="space-y-2">
                            {results.transactions.patterns.map((pattern: any, index: number) => (
                              <div key={index} className="p-3 border rounded-lg">
                                <div className="flex items-center justify-between">
                                  <span className="font-medium">{pattern.type.replace('_', ' ').toUpperCase()}</span>
                                  <Badge variant={pattern.risk === 'high' ? 'destructive' : 'secondary'}>
                                    {pattern.risk}
                                  </Badge>
                                </div>
                    ...
                    <p className="text-muted-foreground">Run transaction analysis to see detailed results</p>
                  )}
                </CardContent>
              </Card>
            </TabsContent>
                ...
                <CardContent>
                  {results.findings && results.findings.length > 0 ? (
                      ...
                      {results.findings.map((finding: any, index: number) => (
                        <div key={index} className="p-4 border rounded-lg">
                          ...
                          <div className="text-sm text-muted-foreground">
                            Type: {finding.type} • Source: {finding.source || 'AI Analysis'}
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="text-center py-8">
                      ...
                      <h3 className="text-lg font-medium">No Risk Patterns Detected</h3>
                      <p className="text-muted-foreground">This address appears to have normal transaction patterns</p>
                    </div>
                  )}
                </CardContent>
              </Card>
                  ...
                  {results.recommendations ? (
                    ...
                    <div className="space-y-3">
                      <div className="flex items-start gap-3 p-3 bg-muted rounded-lg">
                        <InfoIcon className="h-5 w-5 text-brand-light mt-0.5 flex-shrink-0" />
                        <span>Continue monitoring for unusual activity patterns</span>
                      </div>
                      <div className="flex items-start gap-3 p-3 bg-muted rounded-lg">
                        <InfoIcon className="h-5 w-5 text-brand-light mt-0.5 flex-shrink-0" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Screen multiple addresses simultaneously</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <Textarea
                ...
                rows={4}
              />
              <Button onClick={handleBulkScreening} disabled={isLoading} className="w-full">
                {isLoading ? <RefreshIcon className="mr-2 h-4 w-4 animate-spin" /> : <SecurityIcon className="mr-2 h-4 w-4" />}
                Screen Addresses
              </Button>
              {bulkResults && (
                <div className="mt-4">
                  <div className="grid grid-cols-2 gap-2 mb-4">
                    <div className="text-center p-2 bg-muted rounded">
                      <div className="font-bold">{bulkResults.summary.totalAddresses}</div>
                      <div className="text-xs">Total</div>
                    </div>
                    <div className="text-center p-2 bg-red-100 dark:bg-red-900/20 rounded">
                      <div className="font-bold text-red-600">{bulkResults.summary.highRisk}</div>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Check names against PEP databases</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <Input
              ...
              />
              <Button onClick={handlePEPCheck} disabled={isLoading} className="w-full">
                {isLoading ? <RefreshIcon className="mr-2 h-4 w-4 animate-spin" /> : <SearchIcon className="mr-2 h-4 w-4" />}
                Check PEP Status
              </Button>
              {pepResults && (
