...
export class SupervisorAgent {
  ...
  private setupAgentGraph() {
    ...
    this.graph.addNode(
      "blockchain_tracer",
      this.blockchainTracerNode.bind(this),
    );
    this.graph.addNode(
      "sanctions_screener",
      this.sanctionsScreenerNode.bind(this),
    );
    ...
    this.graph.addConditionalEdges("supervisor", this.routeAgent.bind(this), {
  ...
  private async blockchainTracerNode(
    state: typeof AgentState.State,
  ): Promise<Partial<typeof AgentState.State>> {
    ...
    try {
      ...
      if (chain === "solana") {
        ...
        tracingResult = {
          ...
          connectedAddresses:
            this.extractConnectedAddresses(transactionHistory),
  ...
  private async sanctionsScreenerNode(
    state: typeof AgentState.State,
  ): Promise<Partial<typeof AgentState.State>> {
    ...
    try {
      ...
      const screeningResult = {
        ...
        // Combined confidence
        confidence: Math.max(
          chainabuseResult.data.confidence,
          metasleuthResult.data.confidence,
        ),
        timestamp: Date.now(),
      };
  ...
  private async mediaAnalyzerNode(
    state: typeof AgentState.State,
  ): Promise<Partial<typeof AgentState.State>> {
      ...
      const mediaResponse = await model.invoke([
        {
          role: "system",
          content:
            "You are an expert in adverse media analysis for AML compliance.",
        },
  ...
  private async flowVisualizerNode(
    state: typeof AgentState.State,
  ): Promise<Partial<typeof AgentState.State>> {
  ...
  private async coordinatorNode(
    ...
    const reportResponse = await model.invoke([
      {
        role: "system",
        content:
          "You are an expert AML compliance analyst generating investigation reports.",
      },
    ...
    return {
      ...
      messages: [
        new AIMessage(
          `Investigation completed. Final report generated with risk level: ${finalReport.riskLevel}`,
        ),
      ],
    };
  }
  ...
  private detectRapidFirePattern(transactions: any[]): any {
    ...
    for (let i = 0; i < transactions.length - 1; i++) {
      const startTime =
        transactions[i].timestamp || transactions[i].blockTime * 1000;
      let count = 1;

      for (let j = i + 1; j < transactions.length; j++) {
        const txTime =
          transactions[j].timestamp || transactions[j].blockTime * 1000;
  ...
  private determineNextAgent(
    ...
    if (!completedAgents.includes("blockchain_tracer"))
      return "blockchain_tracer";
    if (!completedAgents.includes("sanctions_screener"))
  ...
  private generateSanctionsFindings(result: any): any[] {
    ...
    if (result.labels && result.labels.length > 0) {
      const concerningLabels = result.labels.filter(
        (label: any) =>
          label.type === "sanctions" ||
          label.type === "scam" ||
          label.type === "hack",
      );
  ...
  private generateVisualizationNodes(address: string, tracingData: any): any[] {
    const nodes = [
      { id: address, type: "target", label: address.substring(0, 8) + "..." },
    ];
  ...
  async runInvestigation(input: AgentInput): Promise<any> {
    ...
    const initialState = {
      messages: [
        new HumanMessage(
          `Investigate address: ${input.address}. Type: ${input.investigationType}. ${input.userQuery || ""}`,
        ),
      ],
