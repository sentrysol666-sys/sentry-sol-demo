...
export interface FlowPattern {
  type: 'mixing' | 'layering' | 'structuring' | 'circular' | 'fan_out' | 'fan_in';
  addresses: string[];
  amount: number;
  confidence: number;
  description: string;
...
export class BlockchainTracerAgent {
  ...
  async traceAddress(address: string): Promise<TracingResult> {
    ...
    try {
      ...
      const flowPatterns = await this.analyzeFlowPatterns(address, transactionHistory);
      
      // Step 3: Identify risk indicators
      const riskIndicators = await this.identifyRiskIndicators(transactionHistory, flowPatterns);
      
      // Step 4: Map connected entities
      const connectedEntities = await this.mapConnectedEntities(address, transactionHistory);
      
      // Step 5: Perform temporal analysis
      const temporalAnalysis = await this.performTemporalAnalysis(transactionHistory);
      
      // Step 6: Calculate overall confidence
      const confidence = this.calculateConfidence(transactionHistory, flowPatterns, riskIndicators);
  ...
  private async getTransactionHistory(address: string): Promise<any[]> {
    try {
      ...
      if (this.isSolanaAddress(address)) {
        ...
        if (heliusClient) {
          const result = await mcpManager.callTool('helius', 'get_transaction_history', {
            address,
            limit: 100
          });
      ...
      if (this.isEthereumAddress(address)) {
        ...
        if (etherscanClient) {
          const result = await mcpManager.callTool('etherscan', 'get_transaction_history', {
            address,
            limit: 100
          });
          return result.transactions || [];
  ...
  private async analyzeFlowPatterns(address: string, transactions: any[]): Promise<FlowPattern[]> {
    ...
    try {
      const response = await this.model.invoke([
        { role: "system", content: "You are an expert blockchain forensics analyst specializing in transaction flow analysis." },
        { role: "user", content: prompt }
      ]);

      // Parse AI response and generate structured patterns
      return this.extractPatternsFromResponse(response.content as string, transactions);
    } catch (error) {
  ...
  private async identifyRiskIndicators(transactions: any[], patterns: FlowPattern[]): Promise<RiskIndicator[]> {
  ...
  private async mapConnectedEntities(address: string, transactions: any[]): Promise<ConnectedEntity[]> {
  ...
  private async performTemporalAnalysis(transactions: any[]): Promise<TemporalAnalysis> {
  ...
  private calculateConfidence(transactions: any[], patterns: FlowPattern[], indicators: RiskIndicator[]): number {
  ...
  private extractPatternsFromResponse(response: string, transactions: any[]): FlowPattern[] {
  ...
  private generateMockPatterns(transactions: any[]): FlowPattern[] {
    ...
    if (Math.random() > 0.7) {
      patterns.push({
        ...
        confidence: 0.6,
        description: 'Fan-out pattern detected - single source to multiple destinations'
      });
    }
  ...
  private identifyActivityPeriods(periods: Map<string, any[]>): ActivityPeriod[] {
    const activityPeriods: ActivityPeriod[] = [];
    
    periods.forEach((txs, date) => {
      if (txs.length > 5) { // Threshold for "active" period
