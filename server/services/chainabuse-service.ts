...
class ChainabuseService {
  ...
  async checkSanctionedAddress(address: string): Promise<ChainabuseResponse> {
    try {
      const response = await fetch(`${this.baseUrl}/address/${address}/sanctions`, {
        method: 'GET',
        headers: {
      ...
      if (!response.ok) {
        throw new Error(`Chainabuse sanctions check failed: ${response.statusText}`);
      }
      ...
      console.log(`✅ Chainabuse sanctions check for ${address}:`, formattedResponse.data);
  ...
  async getAddressReports(address: string): Promise<any[]> {
    try {
      const response = await fetch(`${this.baseUrl}/address/${address}/reports`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${this.apiKey}`,
          'Content-Type': 'application/json',
      ...
      if (!response.ok) {
        throw new Error(`Chainabuse reports fetch failed: ${response.statusText}`);
      }
  ...
  async createReport(reportData: ChainabuseCreateReportRequest): Promise<ChainabuseReportResponse> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Chainabuse report creation failed: ${response.statusText}`);
      }
  ...
  async bulkCheckSanctions(addresses: string[]): Promise<ChainabuseResponse[]> {
    try {
      ...
      if (!response.ok) {
        throw new Error(`Chainabuse bulk sanctions check failed: ${response.statusText}`);
      }
