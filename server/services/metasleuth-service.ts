...
class MetaSleuthService {
  ...
  constructor() {
    this.walletScreeningApiKey =
      process.env.METASLEUTH_WALLET_SCREENING_API_KEY || "";
    this.addressLabelApiKey =
      process.env.METASLEUTH_ADDRESS_LABEL_API_KEY || "";
  ...
  ): Promise<MetaSleuthWalletScreeningResponse> {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `MetaSleuth wallet screening failed: ${response.statusText}`,
        );
      }

      const result =
        (await response.json()) as MetaSleuthWalletScreeningResponse;
      console.log(
        `✅ MetaSleuth wallet screening for ${address}:`,
        result.data,
      );
      return result;
    } catch (error) {
  ...
  async getAddressLabels(
    address: string,
  ): Promise<MetaSleuthAddressLabelResponse> {
    try {
      const response = await fetch(
        `${this.baseUrl}/aml/v1/address-labels/${address}`,
        {
          method: "GET",
          headers: {
      ...
      if (!response.ok) {
        throw new Error(
          `MetaSleuth address labels failed: ${response.statusText}`,
        );
      }
  ...
  async bulkScreenWallets(
    addresses: string[],
    chain: string = "ethereum",
  ): Promise<MetaSleuthWalletScreeningResponse[]> {
    try {
      const response = await fetch(
        `${this.baseUrl}/aml/v1/bulk-wallet-screening`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (!response.ok) {
        throw new Error(
          `MetaSleuth bulk screening failed: ${response.statusText}`,
        );
      }
