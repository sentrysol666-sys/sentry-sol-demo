...
export const investigateAddressWithAgents: RequestHandler = async (
  req,
  res,
) => {
  try {
    ...
    if (!isSolanaAddress && !isEthereumAddress) {
      return res.status(400).json({
        success: false,
        error:
          "Invalid address format. Please provide a valid Solana or Ethereum address.",
      });
    }

    console.log(
      `🧠 API: Starting multi-agent investigation for address: ${address}`,
    );

    const result = await investigationService.investigateAddressWithAgents(
      address,
      investigationType,
    );
...
export const performBlockchainTracing: RequestHandler = async (req, res) => {
  try {
    ...
    console.log(
      `🔍 API: Performing blockchain tracing for address: ${address}`,
    );
...
export const generateFlowVisualization: RequestHandler = async (req, res) => {
  try {
    ...
    console.log(
      `📊 API: Generating flow visualization for address: ${address}`,
    );
...
export const callMCPTool: RequestHandler = async (req, res) => {
  try {
    ...
    console.error(
      `Error calling tool ${req.params.toolName} on ${req.params.serverKey}:`,
      error,
    );
    res.status(500).json({
      success: false,
