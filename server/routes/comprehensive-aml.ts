...
router.post("/screen-wallet", async (req, res) => {
  try {
    ...
    const screeningResult = {
      ...
      recommendations: generateScreeningRecommendations(
        metasleuthResult.data,
        chainabuseResult.data,
      ),
    };
...
function generateScreeningRecommendations(
  ...
  if (chainabuse.sanctioned) {
    recommendations.push(
      "IMMEDIATE ACTION: Address is sanctioned - freeze all transactions",
    );
  ...
  if (metasleuth.mixerServices.length > 0) {
    recommendations.push(
      "Investigate mixer service usage for money laundering",
    );
  }

  if (metasleuth.blacklistMatch) {
    recommendations.push(
      "Address found on blacklist - review transaction history",
    );
  }
...
function calculateRiskMetrics(transactions: any[], patterns: any[]): any {
  return {
    ...
    overallRisk: Math.min(
      (patterns.length * 0.2 +
        calculateVolumeRisk(transactions) +
...
function calculateTimespan(transactions: any[]): any {
  ...
  const timestamps = transactions.map(
    (tx) => tx.timestamp || tx.blockTime * 1000,
  );
